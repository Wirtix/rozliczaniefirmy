...
function headerRow(doc: PDFDocument, label: string, value: string) {
  doc.font('Helvetica').fontSize(11).text(label, { continued: true, width: 200 });
...
export async function buildInvoice(payload: InvoicePayload): Promise<Buffer> {
  ...
  doc.font('Helvetica').fontSize(11).text(`${COMPANY.city}, dn. ${issueDate.format('DD.MM.YYYY')}`, {
    align: 'right',
  });

  doc.moveDown(0.6);
  doc.font('Helvetica-Bold').fontSize(16).text('RACHUNEK DO UMOWY ZLECENIE', { align: 'center' });
  doc.font('Helvetica').fontSize(11).text(`nr ${payload.number} z dnia ${issueDate.format('DD.MM.YYYY')}`, {
    align: 'center',
  });

  doc.moveDown();
  doc.font('Helvetica-Bold').fontSize(13).text('Zleceniobiorca:');
  doc.moveDown(0.2);
  ...
  doc.moveDown(0.4);
  doc.font('Helvetica-Bold').fontSize(13).text('Zleceniodawca:');
  doc.moveDown(0.2);
  ...
  doc.moveDown(0.5);
  doc.font('Helvetica-Bold').fontSize(13).text('ROZLICZENIE');
  doc.moveDown(0.2);
  ...
  labels.forEach((label, index) => {
    ...
    doc.font('Helvetica').text(`${index + 1}.`, { continued: true, width: 20 });
    doc.text(label, { continued: true, width: 320 });
    doc.font('Helvetica-Bold').text(currency(Number(value)));
  });
  ...
  const baseY = doc.y;
  dottedLine(doc, '(..........................................................)', 220);
  doc.font('Helvetica').text('Podpis Zleceniobiorcy', baseY + 16, { continued: false });
  ...
  doc.moveDown(2.2);
  doc.font('Helvetica-Bold').text('ZESTAWIENIE WYKONANYCH ZLECEŃ');
  doc.font('Helvetica').text(payload.tasksDescription || '-', { lineGap: 2 });
